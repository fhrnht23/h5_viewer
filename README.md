...
- **Работа с большими файлами.** Группы загружаются постранично, datasets читаются ограниченными
  срезами, а сравнение и экспорт выполняются блоками. Логический и физический размеры datasets
  видны в навигаторе; рекурсивный размер группы рассчитывается по запросу без чтения данных.
...
| Метаданные | Атрибуты, dtype, shape, layout, chunks, compression, логический и физический размеры, fill value, object token и DDL-представление |
...
Установщики и переносимые сборки публикуются на странице
**[GitHub Releases](https://github.com/fhrnht23/h5_viewer/releases/latest)**. Python для них не
нужен.

- **Windows x86-64:** запустите файл `H5Viewer-…-Windows-x86_64-Setup.exe`.
- **macOS Apple Silicon / Intel:** выберите архив своей архитектуры, распакуйте его и перенесите
  `H5Viewer.app` в «Программы».
- **Linux x86-64:** разрешите запуск командой `chmod +x H5Viewer-*.AppImage`, затем откройте
  AppImage.

Файл `SHA256SUMS.txt` в каждом релизе позволяет проверить целостность загрузок. Текущие сборки
Windows и macOS пока не имеют коммерческой подписи издателя, поэтому система может показать
стандартное предупреждение о приложении, загруженном из интернета.
...
Результат появится в каталоге `dist/`. GitHub Actions проверяет проект, собирает desktop-пакеты
отдельно для Windows, macOS и Linux и публикует их в Releases по тегу версии. Подробности приведены в
